  ...
  class File { 
    ...
    public function getFileName(){
      if(is_array($this->newFileName)){
         foreach($this->newFileName as $value){
            $fileName[] = rtrim($this->path, '/').'/'.$value;
         }
      }else{
    ...
    private function copyFile() {
      if(!$this->errorNum) {
        $path  = \Yii::$app->basePath.rtrim($this->path, '/').'/';
        if(!file_exists($path)){
        ...
        // $this -> hire = move_uploaded_file($this->tmpFileName, $path);
